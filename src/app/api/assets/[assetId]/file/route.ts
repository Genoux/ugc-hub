...
import { creatorPortfolioAssets, creatorProfileAssets, submissionAssets } from "@/db/schema";
 ...
 * Resolves against submissionAssets, creatorProfileAssets, and creatorPortfolioAssets.
 */
export async function GET(_request: Request, { params }: { params: Promise<{ assetId: string }> }) {
  const { assetId } = await params;

  const asset =
    (await db.query.submissionAssets.findFirst({ where: eq(submissionAssets.id, assetId) })) ??
